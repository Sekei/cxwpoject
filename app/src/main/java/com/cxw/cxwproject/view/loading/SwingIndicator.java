...
public class SwingIndicator extends View{
	...
	public SwingIndicator(Context context, AttributeSet attrs) {
		...
		circleRadius = (int)a.getDimension(R.styleable.swingindicator_swingcircleRadius, paramsCreator.getDefaultCircleRadius());
		swingRadius = (int)a.getDimension(R.styleable.swingindicator_swingRadius, paramsCreator.getDefaultCircleSpacing());
		
		circleColor = (int)a.getColor(R.styleable.swingindicator_swingcircleColor, 0);
		if(circleColor == 0)
			circleColor = a.getResourceId(R.styleable.swingindicator_swingcircleColor, 0);
		if(circleColor == 0)
			circleColor = 0xFF0189c5;
		
		int cycle = a.getInt(R.styleable.swingindicator_swingcycle, 400);//周期，默认为2秒
		cycle = cycle/2;
