...
public class CountDownView extends View {
	...
	public CountDownView(Context context, AttributeSet attrs) {
		...
		backgroundColor = ta.getColor(R.styleable.CountDownView_timer_background_color, BACKGROUND_COLOR);
		borderWidth = ta.getDimension(R.styleable.CountDownView_timer_border_width, BORDER_WIDTH);
		borderColor = ta.getColor(R.styleable.CountDownView_timer_border_color, BORDER_COLOR);
		text = ta.getString(R.styleable.CountDownView_timer_text);
		if (text == null) {
			text = TEXT;
		}
		textSize = ta.getDimension(R.styleable.CountDownView_timer_text_size, TEXT_SIZE);
		textColor = ta.getColor(R.styleable.CountDownView_timer_text_color, TEXT_COLOR);
		ta.recycle();
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		RectF rectF;
		if (width > height) {
			rectF = new RectF(width / 2 - min / 2 + borderWidth / 2, 0 + borderWidth / 2,
					width / 2 + min / 2 - borderWidth / 2, height - borderWidth / 2);
		} else {
			rectF = new RectF(borderWidth / 2, height / 2 - min / 2 + borderWidth / 2, width - borderWidth / 2,
	...
	public void start(final long startTimer) {
		if (listener != null) {
			listener.onStartCount();
		}
		new CountDownTimer(3600, 36) {
			@Override
			public void onTick(long millisUntilFinished) {
				progress = ((3600 - millisUntilFinished) / 3600f) * 360;
				TLog.d(TAG, "progress:" + progress);
				invalidate();
			}
